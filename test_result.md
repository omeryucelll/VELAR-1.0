...
agent_communication:
  ...
  - agent: "main"
    message: "Fixed work order creation issues: Modified backend to accept custom process_steps for work orders, updated frontend to require both project and step selection, removed auto-project creation logic. URGENT BUG FIX: Dashboard was showing incorrect current step info (project defaults instead of actual custom steps). Fixed dashboard logic to fetch current step from actual process instances. Need to test dashboard current step display."
  - agent: "testing"
